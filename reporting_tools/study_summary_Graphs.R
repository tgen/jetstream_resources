...
ggplot(tasks, aes(x=Group, y=Total_CPU_Hours)) + 
  geom_jitter() + 
  coord_flip()
ggsave(file=paste(study_name, "_TotalCPUhours_by_Group_per_Project.png", sep=""), dpi=150)

ggplot(tasks, aes(x=Group, y=Total_Elapsed_Hours)) + 
  geom_jitter() + 
  coord_flip()
ggsave(file=paste(study_name, "_TotalElapsedHours_by_Group_per_Project.png", sep=""), dpi=150)

ggplot(tasks, aes(x=Group, y=Max_Task_Elapsed_Hours)) + 
  geom_jitter() + 
  coord_flip()
ggsave(file=paste(study_name, "_MaxTaskElapsedHours_by_Group_per_Project.png", sep=""), dpi=150)

ggplot(tasks, aes(x=Group, y=Max_Task_CPU_Hours)) + 
  geom_jitter() + 
  coord_flip()
ggsave(file=paste(study_name, "_MaxTaskCPUhours_by_Group_per_Project.png", sep=""), dpi=150)

ggplot(projects, aes(x=Total_CPU_Hours, y=Total_Elapsed_Hours, color=Tasks)) + 
  geom_point() + 
  scale_colour_gradientn(colours = rainbow(10))
ggsave(file=paste(study_name, "_ElapsedTime_by_Task_per_Group.png", sep=""), dpi=150)
