...
if [ -e v92 ]
then
    echo "COSMIC v92 folder exists, exiting to prevent overwrite"
    exit 1
else
    echo "COSMIC v92 folder NOT found, creating and moving into it now"
    mkdir -p v92
    cd v92
fi
...
RESPONSE=`curl -H "Authorization: Basic ${TOKEN}" https://cancer.sanger.ac.uk/cosmic/file_download/GRCh38/cosmic/v92/VCF/CosmicCodingMuts.vcf.gz`
DOWNLOAD_KEY=`echo $RESPONSE | sed 's/"}//g' | sed 's/{"url":"https:\/\/cog.sanger.ac.uk\/cosmic\/GRCh38\/cosmic\/v92\/VCF\/CosmicCodingMuts.vcf.gz//g'`
# Download Cosmic Coding Mutations file (Authentication token expires after 1 hour)
curl "https://cog.sanger.ac.uk/cosmic/GRCh38/cosmic/v92/VCF/CosmicCodingMuts.vcf.gz${DOWNLOAD_KEY}" > CosmicCodingMuts_v92_GRCh38.vcf.gz

# The downloaded file is not bgzip so it needs to be decompressed with gunzip before processing
gunzip CosmicCodingMuts_v92_GRCh38.vcf.gz
...
bcftools view -H CosmicCodingMuts_v92_GRCh38.vcf | cut -f1 | sort | uniq
echo
...
bcftools view \
    --threads 4 \
    --output-type z \
    --output-file CosmicCodingMuts_v92_GRCh38.vcf.bgz \
    CosmicCodingMuts_v92_GRCh38.vcf
bcftools index --tbi CosmicCodingMuts_v92_GRCh38.vcf.bgz
...
bcftools annotate \
	...
	CosmicCodingMuts_v92_GRCh38.vcf.bgz
...
bcftools reheader \
	...
	bcftools view \
	--output-type b \
	--output-file CosmicCodingMuts_v92_hg38tgen.bcf
bcftools index --threads 4 CosmicCodingMuts_v92_hg38tgen.bcf

rm temp_*
rm CosmicCodingMuts_v92_GRCh38.vcf.bgz.tbi
rm CosmicCodingMuts_v92_GRCh38.vcf.bgz
rm CosmicCodingMuts_v92_GRCh38.vcf
...
RESPONSE=`curl -H "Authorization: Basic ${TOKEN}" https://cancer.sanger.ac.uk/cosmic/file_download/GRCh38/cosmic/v92/VCF/CosmicNonCodingVariants.vcf.gz`
DOWNLOAD_KEY=`echo $RESPONSE | sed 's/"}//g' | sed 's/{"url":"https:\/\/cog.sanger.ac.uk\/cosmic\/GRCh38\/cosmic\/v92\/VCF\/CosmicNonCodingVariants.vcf.gz//g'`
curl "https://cog.sanger.ac.uk/cosmic/GRCh38/cosmic/v92/VCF/CosmicNonCodingVariants.vcf.gz${DOWNLOAD_KEY}" > CosmicNonCodingMuts_v92_GRCh38.vcf.gz

# The downloaded file is not bgzip so it needs to be decompressed with gunzip before processing
gunzip CosmicNonCodingMuts_v92_GRCh38.vcf.gz

# Check the contigs in the file, for matching the contigs need to match and coming from ensemble they will be 1 not chr1, etx..
echo
echo "Unique contig list in Cosmic Coding Mutations File:"
bcftools view -H CosmicNonCodingMuts_v92_GRCh38.vcf | cut -f1 | sort | uniq
echo
### As expected only contigs in the VCF are 1-22, X, Y, MT in both files
...
bcftools view \
    --threads 4 \
    --output-type z \
    --output-file CosmicNonCodingMuts_v92_GRCh38.vcf.bgz \
    CosmicNonCodingMuts_v92_GRCh38.vcf
bcftools index --tbi CosmicNonCodingMuts_v92_GRCh38.vcf.bgz
...
bcftools annotate \
	...
	--output-type b \
	--output temp_renamed.bcf \
	CosmicNonCodingMuts_v92_GRCh38.vcf.bgz
bcftools index --threads 4 temp_renamed.bcf
...
bcftools reheader \
	...
	bcftools view \
	--output-type b \
	--output-file CosmicNonCodingMuts_v92_hg38tgen.bcf
bcftools index --threads 4 CosmicNonCodingMuts_v92_hg38tgen.bcf

rm temp_*
rm CosmicNonCodingMuts_v92_GRCh38.vcf.bgz.tbi
rm CosmicNonCodingMuts_v92_GRCh38.vcf.bgz
rm CosmicNonCodingMuts_v92_GRCh38.vcf
