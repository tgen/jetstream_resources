...
# Usage: ./build_clinvar_20201020.sh
...
wget ftp://ftp.ncbi.nlm.nih.gov/pub/clinvar/vcf_GRCh38/archive_2.0/2020/clinvar_20201020.vcf.g*

# Remove local NIH path from MD5 file
sed -i 's/\/panfs\/pan1\/clintest\/ftp_test_prod\/vcf\/vcf_GRCh38\///g' clinvar_20201020.vcf.gz.md5

# Check MD5 checksums
CHECKSUM_STATUS=`md5sum --check clinvar_20201020.vcf.gz.md5 | cut -d" " -f2`
...
then
  echo
  md5sum clinvar_20201020.vcf.gz
  cat clinvar_20201020.vcf.gz.md5
...
zcat clinvar_20201020.vcf.gz | cut -f1 | grep -v "#" | sort | uniq
# Found 1-22,X,Y,MT, and NW_009646201.1, which is a GRCh38.p13 patch for the ABO*A1.02 allele

## Remove the NW_009646201.1 patch and convert the contig names in clinvar_20201020.vcf.gz
bcftools filter \
	--threads 4 \
	--targets ^NW_009646201.1  \
	--output-type b \
	--output temp_droppedContigs.bcf \
	clinvar_20201020.vcf.gz
...
bcftools reheader \
	...
	bcftools view \
	--output-type b \
	--output-file clinvar_20201020_hg38tgen.bcf
bcftools index --threads 4 clinvar_20201020_hg38tgen.bcf
