...
CLINVAR_YEAR=${CLINVAR_VERSION:0:4}
...
then
  echo
  md5sum clinvar_${CLINVAR_VERSION}.vcf.gz
  cat clinvar_${CLINVAR_VERSION}.vcf.gz.md5
...
# Check contig sequences to determine if they match our hg38_tempe reference genome with UCSC contif names
zcat clinvar_${CLINVAR_VERSION}.vcf.gz | cut -f1 | grep -v "#" | sort | uniq
# Found 1-22,X,Y,MT, and NW_009646201.1, which is a GRCh38.p13 patch for the ABO*A1.02 allele

## Remove the NW_009646201.1 patch and convert the contig names in clinvar_${CLINVAR_VERSION}.vcf.gz
bcftools filter \
	--threads 4 \
	--targets ^NW_009646201.1  \
	--output-type b \
	--output temp_droppedContigs.bcf \
	clinvar_${CLINVAR_VERSION}.vcf.gz
...
bcftools reheader \
	...
	bcftools view \
	--output-type b \
	--output-file clinvar_${CLINVAR_VERSION}_hg38_tempe.bcf
bcftools index --threads 4 clinvar_${CLINVAR_VERSION}_hg38_tempe.bcf
