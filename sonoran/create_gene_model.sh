...
GTF_DOWNLOAD_BASE=`basename ${GENE_MODEL_DOWNLOAD_LINK}`

# UU does not provide an md5 for the gtf, nothing to check :(

# Determine expected GTF filename
echo "GTF_FILE=$GENE_MODEL_FILENAME" >> README
GTF_FILE=$GENE_MODEL_FILENAME

# Fasta does not have the chrUn contigs, removing them from the gtf for now
cat $GTF_DOWNLOAD_BASE | grep -v ^chrUn > $GTF_FILE
